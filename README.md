...
| 3 | `Filter` / `Filter1` | **조건 분기** — 1억 이상 / 1억 미만 두 갈래 |
...
| 8 | **무료 플랜 범위** | 셀프호스팅 시 실행 횟수 제한 없음 (서버는 직접 준비) | 월 1,000 operations 무료. 본 과제는 모듈 3~4개 × 실행이라 **15분 주기로 돌리면 월 한도를 초과** |
| 9 | **설치 / 접근성** | 셀프호스팅·Docker 등 초기 환경 구성 필요 | **가입 즉시 브라우저에서 사용**. 설치 불필요 |
...
| 선정 이유 | 설명 |
|---|---|
| 서버 불필요 | 24시간 켜둘 PC나 서버 없이 클라우드에서 자동 실행됨. n8n 셀프호스팅은 PC를 꺼두면 알림이 멈춤 |
